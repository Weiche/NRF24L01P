...
network_info_t Network = { .network_addr = { 2, 3, 4, 5 }, .node_table = NULL };
...
int NRF24_Net_Init(NRF24_InitTypedef* ifnet, const uint8_t* network_mac,
...
__attribute__((__weak__)) void NRF24_Net_ReceiveCallback(
		const uint8_t* payload, const uint32_t len) {
}

void NRF24_Net_ReceiveTask(void) {
	uint8_t data[32], status, pipe;
	uint32_t length;
